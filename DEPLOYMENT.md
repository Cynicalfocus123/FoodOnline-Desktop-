...
After migration, Refer & Earn is fully Laravel-authoritative and shared by Customer, Supplier, and Partner accounts: codes are permanent and unique, invite resolution is read-only, valid cross-role registration attribution is idempotent, promotions remain account-bound, qualified rewards are idempotent, and full refunds revoke unused rewards or route redeemed rewards to review. The matched frontend includes the same dashboard/activity/coupons and direct invite refresh for all three public account types; the Admin Referrals module includes server-owned mixed-role list/search/filter/pagination, dedicated detail/review routes, settings, rewards, referral-scoped notifications, and audit history.
...
Never run `migrate:fresh`, reset, truncate, or seed against production. Then verify `/api/v1/health`; Customer, Supplier, and Partner registration; token-authenticated `/api/v1/auth/me`; direct invite/refresh for codes owned by all three account types; all nine valid referrer/referred role combinations plus invalid and self-code validation; all three referral dashboard/activity/coupon routes; account-bound coupon rejection for another account; delivered/paid or COD qualification; full-refund revocation; mixed-role Admin Referral list/detail/settings; and direct Admin Customer/Supplier/Partner detail with original fields plus customer address/payment sections. Inspect the timestamped Laravel/PHP log if any route returns 500. These are manual production actions and smoke tests; local package creation is not evidence they occurred.
