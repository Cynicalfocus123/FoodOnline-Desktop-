...
After both packages succeed, the command removes every other root-level `.zip` from the dedicated external release directory. Operators must see only the latest canonical frontend/backend pair; do not preserve legacy, alternate, timestamped, or superseded ZIP copies in that folder.
