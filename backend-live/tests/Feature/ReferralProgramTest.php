...
class ReferralProgramTest extends TestCase
{
    ...
    public function test_production_style_upgrade_restores_the_required_schema_without_touching_existing_referrals(): void
    {
        $referrer = $this->registerAccount('supplier', 'upgrade-referrer@example.test');
        $friend = $this->registerAccount('partner', 'upgrade-friend@example.test', ReferralCode::query()->where('user_id', $referrer['user']->id)->value('code'));
        $referralId = Referral::query()->where('referred_user_id', $friend['user']->id)->value('id');

        Schema::table('referrals', fn ($table) => $table->dropColumn('review_note'));
        $upgrade = require database_path('migrations/2026_07_23_700200_upgrade_referral_schema_for_existing_installations.php');
        $upgrade->up();

        $this->assertTrue(app(ReferralSchema::class)->isReady());
        $this->assertDatabaseHas('referrals', ['id' => $referralId, 'referrer_user_id' => $referrer['user']->id, 'referred_user_id' => $friend['user']->id]);
        $this->artisan('referrals:diagnose')
            ->expectsOutput('Core schema ready: yes')
            ->expectsOutput('Full schema ready: yes')
            ->expectsOutput('Missing tables/columns: none')
            ->expectsOutput('Active referral program exists: yes')
            ->assertExitCode(0);
    }
