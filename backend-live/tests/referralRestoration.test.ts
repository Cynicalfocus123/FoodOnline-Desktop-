...
test("Admin referrals use real mixed-role filters, detail actions, and independently retryable detail sections", () => {
  ...
  assert.match(adminPanel, /Search by name, email, or code/);
  assert.match(adminPanel, /All referral statuses/);
  assert.match(adminPanel, /All review statuses/);
  assert.match(adminPanel, /grid-cols-1 gap-3/);
  assert.match(adminPanel, /2xl:grid-cols/);
  assert.doesNotMatch(adminPanel, new RegExp(["All referral statu", "ss", "|All review statu", "ss", "|Search referrer or ", "friend"].join("")));
