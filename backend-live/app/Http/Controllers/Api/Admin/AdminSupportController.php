...
{
    public function index(Request $request): JsonResponse { return response()->json(['data' => SupportTicket::query()->with(['user', 'order', 'media.upload'])->latest('last_message_at')->paginate(25)]); }
    public function show(SupportTicket $ticket): JsonResponse { return response()->json(['ticket' => $ticket->load(['user', 'order', 'messages', 'media.upload'])]); }
