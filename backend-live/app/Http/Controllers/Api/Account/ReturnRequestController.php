...
{
    public function index(Request $request): JsonResponse { return response()->json(['data' => $request->user()->returnRequests()->with(['items.orderItem', 'media.upload'])->latest()->paginate(20)]); }
    ...
    public function show(Request $request, ReturnRequest $returnRequest): JsonResponse { abort_unless($returnRequest->user_id === $request->user()->id, 404); return response()->json(['return' => $returnRequest->load(['items.orderItem', 'order', 'user', 'media.upload'])]); }
