...
{
    public function favorites(Request $request): JsonResponse { $media = app(\App\Services\Catalog\CategoryMediaUrl::class); return response()->json(['data' => $request->user()->favorites()->with(['product.category', 'product.primaryMedia'])->latest()->get()->map(fn ($favorite) => ['product_uuid' => $favorite->product?->uuid, 'product_slug' => $favorite->product?->slug, 'product_name' => $favorite->product?->name, 'product_image_url' => $media->make($favorite->product?->primaryMedia?->path), 'available' => $favorite->product?->status === 'published', 'created_at' => $favorite->created_at?->toIso8601String()])]); }
    ...
    public function savedItems(Request $request): JsonResponse { $media = app(\App\Services\Catalog\CategoryMediaUrl::class); return response()->json(['data' => $request->user()->savedItems()->with(['variant.product.primaryMedia', 'variant.inventory'])->latest()->get()->map(fn ($item) => ['variant_uuid' => $item->variant?->uuid, 'product_uuid' => $item->variant?->product?->uuid, 'product_slug' => $item->variant?->product?->slug, 'product_name' => $item->variant?->product?->name, 'product_image_url' => $media->make($item->variant?->product?->primaryMedia?->path), 'variant_title' => $item->variant?->title, 'quantity' => $item->quantity, 'price' => $item->variant?->price_amount, 'available' => (bool) ($item->variant?->is_active && $item->variant?->product?->status === 'published'), 'created_at' => $item->created_at?->toIso8601String()])]); }
