...
> **Urgent authentication/Admin repair (2026-07-22):** referral integration had become an unconditional dependency of public registration and managed-user detail while the live referral subsystem was not schema-ready. Core registration and Admin Customer/Supplier/Partner editing now degrade safely until migration, use one validated token/user contract, preserve the original editor, and isolate optional customer collections. Deploy the paired release and run the documented production migration/cache sequence; this repository does not claim that external step was performed.

> **User-address acceptance (2026-07-22):** the permanent acceptance command registers a real customer, saves default Thailand and non-default United States records through the public Laravel Address Book contract, confirms selected-user-only Admin API results, and drives a compiled Admin Customer Edit route in headless Chrome before and after refresh. Phone, delivery-note, country-specific fields, separate cards, and the single Default badge are asserted; both extracted Live ZIPs repeat implementation marker checks.

> **Admin-session and multi-role addresses (2026-07-22):** Admin session hydration now renders a neutral restore state, preserves the current token for retryable `/admin/me` failures, and logs out only on an exact matching-token 401. Customer, Supplier, and Partner all share Address Book persistence and selected-user-only Admin address cards; `npm run test:managed-user-address-acceptance` validates all three roles and cross-role isolation.

> **Address Book country calling codes (2026-07-22):** Address Add/Edit forms reuse the registration phone control and one shared country-calling-code dataset. A selected address country chooses its default calling code, full international paste is split safely, and new values save once as normalized international `phoneNumber` data without changing the address schema.
