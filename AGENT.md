...
- Production catalog mode is now explicit `VITE_CATALOG_SOURCE=hybrid` with `VITE_API_BASE_URL=https://www.api.foodonlines.com/api/v1`. Strict `api` and isolated `local` modes remain available; only explicit hybrid mode retains the complete local storefront when the API is incomplete or temporarily unavailable.
