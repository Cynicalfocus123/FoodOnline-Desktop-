...
- Follow-up truck cutoff fix: `public/images/about/circle-assets/delivery-truck.png` was regenerated from the original full `9.Our teams fulfill and deliver more than 100,000 orders every day (1).png` canvas with only the black background made transparent. The truck image is no longer tight-cropped, and the page CSS caps the full canvas width while using negative bottom offsets to place the visible vehicle without cutting off any part of the truck.
