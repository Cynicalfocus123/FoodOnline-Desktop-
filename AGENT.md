...
## Administrator customer addresses and masked payment methods (2026-07-22)

- Root cause: public address and payment metadata already used the authoritative `users.id` foreign key and existing `UserAddress`/`UserPaymentMethod` relationships, but administrator detail was incomplete. Only the show response conditionally loaded addresses, update responses replaced the editor state without those relations, payment methods were never loaded or serialized, and one nullable React detail object conflated loading/failure/unavailable/confirmed-empty states.
- The existing `/api/v1/admin/users/{user}` detail owner now eagerly loads only the selected customer's default-first addresses and active saved payment methods. Address values remain structured and country-specific. Payment output is an explicit allowlist of ID, owner ID, brand, last four, expiry, default/status, and created time; provider/token references and raw payment credentials are never serialized. No route, migration, table, or parallel storage system was added.
- The Customers edit workspace keeps the existing shell and now shows responsive read-only Saved addresses and Payment methods sections with real stable IDs, complete structured fields, country/phone/default indicators, masked card copy, loading/loaded/empty/error+Retry/unavailable states, direct-route loading, canonical returned-ID checks, role checks, and request-version rejection of stale customer responses.
- Validation passed 77 Node tests; 9 focused Laravel tests / 126 assertions; TypeScript no-emit; changed-PHP syntax; config/route/view cache compilation; a 140-module production build; the 28-route/zero-missing-reference audit; exact frontend mirror SHA-256 parity; backend manifest synchronization; and compiled admin checks with zero token-reference, CVV, or raw technical-error findings. The full Laravel suite was also attempted and retained 53 pre-existing media-URL/throttle failures after 46 passes; it is not claimed as passing.
- The clean release command replaces both external staging folders from current `dist/` and `backend-live/`, verifies the pair, then removes those temporary folders and every noncanonical ZIP. The release directory now contains only the two current archives. Final standard Deflate ZIP32 evidence: frontend 1,036 files / 90,946,526 bytes / SHA-256 `264bd5a45dda1b2d8df9e665921acc22e7f843f6229b59f510ff8825eaab92dd`; backend 291 files / 291,377 bytes / SHA-256 `154c75369948ab87d4e2997e430c1a8a43dd6da04dd6f3d872ca212d6127d123`. CRC, Windows/PHP extraction parity, manifest, path/metadata, and forbidden-content gates passed.
...
- Homepage catalog consumers use one shared, deduplicated refresh. API homepage presentation uses a combined paginated product read rather than an individual product request for every category rail; matching API records overlay local identity once and valid API-only records append once.
- Empty loading skeletons appear only when no usable presentation snapshot exists. Category cards and product-card frames render before image completion and retain existing fallbacks, layout dimensions, mobile scrolling, arrows, See All routes, and category-state protections.
- Homepage See All and category tiles continue using canonical slugs and retained data, so they enter the resolution-aware category page without a false empty-category flash. Task 5 validation passed 62 Node tests, TypeScript no-emit, a 135-module production build, the 27-route audit, exact `dist/`/`frontend-upload/` parity (1,033 files / 91,817,320 bytes), and a fresh frontend-only ZIP extraction. Backend source, deployment files, and the backend ZIP are unchanged by this frontend-only task.

## Registration, admin feedback, and category resolution (2026-07-21)

- The final customer, supplier, and partner registration screen contains only `Thank you for registering.` and the orange `Go to Home` action. It uses the existing home navigation, which clears the completed signup state; backend registration and automatic-auth behavior are unchanged.
- Product, Brand, Category, and Promo Code forms use one polite, green shared administrator success alert after a real create/update response. Create feedback survives the existing Edit-route transition, is auto-cleared after display, and selected product/brand/category media reports a warning—not full success—when uploads remain retryable.
- Promo percentage fields accept human values with a presentation-only `%` suffix and convert through the shared utility (for example, `30` percent to `3000` basis points). Percentage validation is `0.01%` through `100%`; fixed discounts and optional subtotal/cap inputs remain monetary minor-unit values behind a money-formatted UI.
- Category listing resolution is explicit: loading, loaded, filtered-empty, confirmed-empty, and failure are distinct. `Products are coming soon` is rendered only after final resolution confirms zero products; loading never claims zero. Valid local/hybrid products render before API completion, canonical See All/category links use stable slugs, and stale requests cannot overwrite the active category route.
...
- Portable ZIP rule: create both production archives only with `npm run release:zips`. Every archive entry must use `/`, never Windows `\`; archive contents stay at ZIP root with no wrapper folder. The release command must fail before publishing on backslash paths, absolute/traversal paths, duplicates, missing/extra files, size mismatches, forbidden frontend paths, or failed extraction verification. Do not use `Compress-Archive` or `ZipFile.CreateFromDirectory` for production packages.
...
- Portable archive verification: backend ZIP is 268,701 bytes / 274 files / SHA-256 `1a66ed0c8d17c26edc7a991555047c53c4f3d7ed9a4545a08b52dab278034893`; frontend ZIP is 90,945,935 bytes / 1,033 files / SHA-256 `777bccb933e1c579add98b0b26e8bee27e19c7b159d8f9ee9e17c052a3dbf429`. Both have zero backslash entries, missing, extra, size-mismatched, unsafe, or forbidden paths, and both pass full extraction verification.
...
- New category, brand, and product forms accept images before the parent record exists. The browser keeps selected files and previews temporarily, supports replacement/removal before save, creates the parent through the unchanged API route, then uploads and associates the files automatically. Existing records still upload, replace, and remove immediately through the same managed-media backend. No media/database migration or new API route was added; operational review/return/support media continues to attach to its already-created workflow record, and no unsupported supplier/partner/customer image schema was invented.
- `catalog:backfill-categories` now restores every soft-deleted category in place before inserting only genuinely missing original slugs. Restored IDs, names, hierarchy, order, lifecycle state, placement flags, media, SEO, and product relationships remain intact; active edited categories and Ice cream remain untouched, and new categories append normally. Published + Public + homepage-enabled restored rows immediately re-enter the existing public homepage query and hybrid storefront placement flow.
- Frontend design, styling/theme, public/admin routes, authentication, database schema, and product data are unchanged. Validation passed 36 Node tests, 77 Laravel tests / 619 assertions, TypeScript, changed-PHP syntax, config/route/view cache compilation, repeatable isolated backfill, Vite production build (119 modules), and the 27-route production audit. `dist/` and `frontend-upload/` contain 1,033 files / 91,774,202 bytes with zero missing local references; `backend-live/` contains 273 source files plus its manifest with zero missing, stale, checksum, secret, frontend, or ZIP findings.
- Delivery remains the non-ZIP workflow: regenerate `backend-live/`, commit and push `main`, deploy the private Laravel source and frontend mirror while preserving the live `.env`, `vendor/`, storage/media, database, permissions, and `public_html/api`, then run migrations, category backfill, caches, and live smoke checks. External deployment is reported only from direct evidence.
...
- Final local evidence: 36 Node tests and 77 Laravel tests / 614 assertions passed; TypeScript no-emit/project build, changed-PHP syntax, config/route/view caches, media diagnostics, isolated cleanup, migration apply/rollback/reapply, production build, and audit passed. Vite transformed 119 modules. Composer validation/audit was unavailable because Composer is not installed.
- Generated delivery evidence: `backend-live/` contains 274 source files plus `SHA256SUMS` with zero missing/stale/checksum/secret/frontend/ZIP findings; environment templates and Git placeholder files are excluded while empty writable runtime directories are created for packaging. `dist/` and `frontend-upload/` each contain 1,033 files / 91,769,707 bytes with zero missing, extra, or SHA-256-mismatched files; prohibited compiled-output findings are zero.
...
- `catalog:backfill-categories` and the shared missing-only seeder service insert absent original slugs and the absent legacy alias without updating existing categories, Ice cream, UUIDs, hierarchy, order, state, placement, media, or SEO. Fresh installs use the same safe implementation.
- Admin Categories loads every page with lifecycle views, keeps selection after save, supports optional Images and SEO/Redirects, neutral media states, bottom Save, archive/restore, and archived-only two-button permanent deletion with product/child safeguards.
- Laravel success defines public category existence and placement; local data supplies matching presentation only. Versioned last-known placement prevents API outages from reintroducing hidden categories. Header/mobile navigation and homepage tiles are dynamic; empty categories retain tiles/pages, empty carousels are omitted, and missing/broken media uses a generic fallback.
- Product publication/public queries no longer require media; active-default-variant, SKU, price, category, availability, and currency rules remain. Delivery uses regenerated live folders, preserves the live environment/storage/vendor/permissions/`public_html/api`, and requires the migration, backfill, and cache commands in `DEPLOYMENT.md`. No external deployment is claimed.
- Final local evidence: 33/33 Node tests, 70/70 Laravel tests with 497 assertions, TypeScript no-emit/project build, PHP syntax, config/route/view cache compilation, isolated SQLite migration apply/rollback/reapply, repeatable backfill (16/1 then 0/0), Vite production build with 116 modules, and the 27-route production audit passed. Composer validation/audit was unavailable because no Composer executable is installed; production MySQL and external services were not exercised.
- Generated delivery evidence: `backend-live/` contains 277 payload files plus `SHA256SUMS` with zero missing/stale/checksum/secret/frontend/ZIP findings. `dist/` and `frontend-upload/` each contain 1,033 files / 91,763,747 bytes with zero missing, extra, or hash-mismatched files and zero unsafe frontend paths.

## PRODUCTION API AND BACKEND VISIBILITY RULE (2026-07-17)

- The frontend communicates with the Laravel API internally. Its centralized, environment-driven production base is `VITE_API_BASE_URL=https://api.foodonlines.com/api/v1`; hybrid catalog mode remains `VITE_CATALOG_SOURCE=hybrid`.
- API base URLs, hostnames, endpoint paths, backend paths, server addresses, environment variable names, storage configuration, credentials, tokens, and other infrastructure details must never be displayed in customer or administrator interfaces.
- Raw Laravel, PHP, SQL, database, server, storage, authentication, exception, stack-trace, HTML, or JSON errors must never be rendered directly. Production errors must pass through the shared frontend normalizer and become safe, understandable messages while useful field validation remains visible.
- API configuration must remain centralized and environment-driven. Secrets must never be placed in frontend source, public environment variables, browser logs, builds, deployment mirrors, documentation, or ZIPs.
- Debugging UI such as an “API Target” banner is development-only and prohibited in production. Every future customer and admin change must follow this rule, and every production build must be searched for visible technical labels and unsafe output before deployment.
- The 2026-07-17 admin sign-in failure was caused by the frontend’s obsolete production host. The old value was `https://www.api.foodonlines.com/api/v1`; the corrected value is `https://api.foodonlines.com/api/v1`. The visible API-target banner and settings URL were removed without changing the `/admin` route or backend behavior.
- The shared `src/lib/userFacingError.ts` layer preserves safe validation guidance and converts network, unauthorized-session, temporary-service, and technical response failures into user-facing copy. Production response bodies and secrets are not logged.
- Delivery record: configuration, runtime fallback, request normalization, auth/admin/public error presentation, technical admin diagnostics, UI copy, tests, and project documentation were updated. Validation uses `npm.cmd test`, `npx.cmd tsc --noEmit`, `npm.cmd run build`, `npm.cmd run audit:production`, and `npm.cmd run sync:hostinger`, followed by compiled-output and ZIP inspection.
- Final validation passed: 25/25 Node tests, TypeScript with no errors, Vite 7.3.3 production build with 112 transformed modules, production audit across 27 routes and 1,033 files/91,751,397 bytes, and `dist`/`frontend-upload` parity with 0 missing, stale, or hash-mismatched files. Compiled and extracted searches found 0 old-host files, 1 file containing the corrected internal host, 0 prohibited debug-label files, and 0 secret-pattern files.
- Deployment artifact: `foodonlines-frontend-api-visibility-fix-2026-07-17.zip`, created outside the repository at `D:\Codex projects\FoodOnlines-Live-Releases\foodonlines-frontend-api-visibility-fix-2026-07-17.zip`. Its production files extract directly into `/home/u517869682/domains/foodonlines.com/public_html`.
- Final ZIP inspection: 90,930,139 bytes, 1,031 files, SHA-256 `a52f1d56b55d1b0d9cf1fcf325c8b87c5fd6e981d125255638f40c1d6e0b0f1a`, root entries `.htaccess`, `404.html`, `admin.html`, `assets/`, `favicon.svg`, `images/`, and `index.html`; 0 wrapper, unsafe, `api/`, backend, private-environment, source-map, or secret paths; extraction parity found 0 missing, stale, or hash-mismatched files.
- The frontend ZIP must never contain, delete, replace, or overwrite `/home/u517869682/domains/foodonlines.com/public_html/api`; that directory contains the separately managed working API public entry point.

## Temporary Hybrid Storefront Catalog Compatibility (2026-07-15)

- Production catalog mode is now explicit `VITE_CATALOG_SOURCE=hybrid` with `VITE_API_BASE_URL=https://api.foodonlines.com/api/v1`. Strict `api` and isolated `local` modes remain available; only explicit hybrid mode retains the complete local storefront when the API is incomplete or temporarily unavailable.
- Verified root cause: Phase 5 production selection defaulted to API-only; category tiles came only from API homepage categories, homepage sections came only from those categories and were filtered when empty, and no repository merged the still-present original 16 categories, 240 base products, 15 homepage rows, or 60-item category listing grids.
- `hybridCatalogRepository` loads local and API repositories, preserves approved local order, overlays matching API authority, appends API-only categories/products, deduplicates identities/media/sections/options, preserves local presentation/media when API fields are absent, and returns local content with a safe synchronization warning after API failure/timeout. API-only mode remains strict and never silently falls back.
- Category identity uses shared UUID when both sides have one, then normalized canonical slug. Product identity uses shared UUID, canonical slug, then product SKU only with exact variant identity; listing clones are never collapsed by their shared display slug and display names are never identity.
- Laravel remains authoritative for matched UUID/slug/category/brand/price/availability/stock/variants/SKU/media/nutrition/review/sold-count fields when supplied. Original local descriptions, galleries, recipes, badges, tags, provider/origin display, return copy, and other presentation fields fill only missing API presentation data.
- Hybrid detail lookup supports exact local IDs, listing-clone IDs, local/API slugs, and API UUIDs. Unknown category/product identifiers return the existing empty/not-found state instead of silently resolving the first local record. Search, related products, filters, and homepage sections merge both sources without removing local records.
- Local-only cart lines are explicit compatibility state. They remain visible and editable but are excluded from Laravel cart hydration, checkout quotes, and order placement and show the existing amber availability presentation. API list cards resolve detail on Add to Cart when necessary so only a real backend variant UUID is submitted. Cart/favorite/saved identity migrates only after an unambiguous product and exact variant match.
- No Laravel source, migration, route, controller, model, service, request, resource, policy, dependency, or `backend-live/` file changes belong to this repair. `frontend-upload/` is regenerated only from the final audited `dist/`; no ZIP or external Hostinger deployment is part of this task. See `docs/hybrid-catalog-compatibility.md`.
...
- Backend carts support hashed guest tokens, authenticated carts, exact product-variant lines, duplicate-variant quantity merging (capped at 99 and clamped to available tracked stock), server hydration, unavailable-line retention, ownership checks, safe clearing, and guest-cart merge after a real email-token login. The current merge rule is `min(99, authenticated quantity + guest quantity)`, then clamped for tracked non-backorder stock.
- Inventory is single-location and variant-owned: quantity on hand, quantity reserved, derived available quantity, tracking/backorder flags, low-stock thresholds, row-locked reservations, release/expiration, shipment consumption, append-only movements, and audited admin adjustments. COD reservations intentionally have no expiration.
- Promotions are admin-managed percentage basis points or fixed minor-unit discounts with case-normalized lookup, dates, usage limits, minimum subtotal, maximum discount, currency checks, product/category restrictions, quote-time errors, transaction-time revalidation, and immutable redemption snapshots.
- Checkout quotes are short-lived, actor/cart-owned, item/address-snapshotted, hash-recorded, server-calculated, and never inventory-reserving. Order creation revalidates catalog publication, variant identity, price, stock, promotion, shipping, tax, settings, total, payment method, and quote expiration.
- Transactional money is stored as integer minor units with explicit currency. Orders are transactional and idempotent by actor plus idempotency key; duplicate requests replay the original order, different-quote key reuse is rejected, purchased cart lines alone are removed, snapshots are immutable, and confirmation mail dispatches after commit.
- Order, payment, and fulfillment status are separate and centralized. COD is the only operational default, stays pending until admin collection, records paid time/amount/history/audit, and has a safe duplicate-collection response. Provider-neutral gateway interfaces and unavailable methods are present; no merchant SDK or real card/wallet provider is configured.
- Raw card/CVV entry is unreachable in checkout; account saved-card metadata is legacy-unverified, non-chargeable, and Add Card/default selection are disabled. Mock phone sessions have no Laravel bearer token and are treated as guest checkout rather than authenticated orders.
- Customer My Orders includes real list/detail/cancel behavior with ownership checks. Guest order details require the one-time access token returned at order creation. Admin Orders, Inventory, Promo Codes, commerce settings, server audit logs, dashboard ecommerce metrics, confirmation dialogs, and responsive loading/empty/error states reuse the existing admin shell.
- Queue confirmation jobs are after-commit; reservation cleanup is scheduled every five minutes and media cleanup remains hourly. `backend-live/` is generated only from authoritative Laravel source; `dist/` and `frontend-upload/` remain delivery mirrors. External Hostinger, MySQL, SMTP, queue workers, cron, R2, DNS, and merchant payment provider are not externally changed or claimed.
- Phase 6 handoff: Phase 7 may add one approved merchant adapter, hosted/tokenized payment sessions, webhooks, refunds, reconciliation, and provider-backed saved methods without changing cart, quote, promotion, inventory, order, customer-history, or admin-operation contracts.

## Public Catalog Cutover and Admin Storefront Verification - Phase 5 (2026-07-13)

- Laravel is now the authoritative public catalog source by default. `VITE_CATALOG_SOURCE=api` selects the asynchronous API repository; `VITE_CATALOG_SOURCE=local` is an explicit local/demo mode and is never activated silently after an API failure.
- Public categories, homepage category tiles, homepage product sections, category listings, search, product detail, related products, R2 media URLs, brands, grocery fields, nutrition, and active variants map from the Laravel `/api/v1/catalog` contract. Product routes use stable slugs; UUIDs remain the product identity for favorites.
- The presentation compatibility layer is isolated from authoritative API fields. It preserves Best Selling display/selection through the existing featured-compatible source, all existing sort and filter controls, product-card fields, product-detail tabs/sections, ratings/reviews/sold-count/provider/badges/tags/recipes/recommendations, promo UI, payment UI, loading/empty/error/retry states, and the existing responsive public design.
- Variant selection changes price, size, availability, and the cart line key. Local cart lines are variant-aware, allow two variants of one product to coexist, hydrate current prices/media from Laravel, retain unavailable lines for removal, and migrate safely from product-keyed legacy values in memory. Favorites use product UUID; Saved for Later retains the exact variant line.
- Existing admin navigation, login, dashboard, Categories, Brands, Products, Variants, Media, Nutrition, R2 workflow, and editor design remain intact. Category and Product editors now include compact read-only Public Storefront Status panels with accessibility reasons and View on Storefront actions. Category homepage/navigation/visibility/default-sort/media and Brand active/order/country/logo controls remain editable; Product featured/publication/category/brand/grocery/variant/default-price/availability/media/nutrition controls remain editable.
- Phase 6 remains deferred: backend/local carts, cart tables/endpoints, orders, promotions, inventory, payments, shipping/tax, reviews, recipes, recommendations, seller offers, and new admin modules are not part of this phase. Exact product and variant UUID identity is ready for that handoff.
- Validation: TypeScript, production build, and production audit passed (`27` routes, `21` lazy chunks, `27` JavaScript files, `0` missing references, `0` placeholder links, `1,033` dist files, `91,691,503` bytes). PHP/Laravel tests were not runnable in this shell because PHP is not installed; no external deployment, migration, R2, DNS, or catalog-source switch was claimed.
- `frontend-upload/` was regenerated from `dist/`. Because the Laravel detail lookup accepts both slug and UUID, `backend-live/` was regenerated from authoritative source with `170` payload files plus `SHA256SUMS`; manifest verification reported zero missing, stale, checksum, secret, frontend, or ZIP findings. Backend ZIP and frontend ZIP remain prohibited.
- Git source, `dist/`, `frontend-upload/`, docs, and the backend mirror are the repository delivery artifacts. External Hostinger frontend/backend remain not updated; Cloudflare R2 remains externally unchanged. The final Phase 5 commit SHA is recorded after commit completion.

## Existing Admin Catalog Portal and R2 Media Workflow — Phase 4 (2026-07-13)

- The standalone React admin entry now extends the existing dashboard with Categories, Brands, and Products navigation while preserving login, users, deletion requests, settings, and user counts. Category hierarchy/SEO/visibility/aliases/media, brand country/logo/state, product grocery fields, variants, ordered media, nutrition, readiness, publish/archive/restore, and practical catalog counts are manageable through typed admin API clients.
- Browser media uploads use protected authorization/completion/cancellation endpoints and short-lived signed PUT URLs. Laravel generates immutable UUID object keys for product images, brand logos, and four category media purposes; permanent database references use `r2://...`, while public responses resolve through `R2_PUBLIC_URL`. Access keys, secrets, endpoints, and presigned URLs are never persisted or bundled into frontend code.
- Added `media_uploads` and `product_nutrition_facts`, `MediaUpload` and `ProductNutritionFact`, R2 filesystem configuration, upload authorization/verification, managed after-commit deletion, hourly/idempotent `media:cleanup`, nutrition admin routes/resources, and public nutrition detail output. JPEG, PNG, and WebP are byte-signature and decode verified with bounded size/dimension rules.
- Validation passed with Laravel 12.63.0, PHP 8.3.32, Composer 2.10.2, and Node 24.14.0/24.18.0: Composer validation/audit, Phase 4 migration apply/rollback/reapply, PHP syntax, config/route caches, TypeScript, production build, production audit, and **52 tests / 351 assertions**. Eight focused Phase 4 test files cover storage status, authorization, completion, category/brand/product media, deletion/cleanup, and nutrition.
- `backend-live/` was regenerated from authoritative source as **170 payload files + `SHA256SUMS` (171 total)** with zero missing, stale, checksum, secret, frontend, or ZIP findings. `dist/` and `frontend-upload/` contain 1,032 files / 91,679,167 bytes. Phase 4 is delivered by the integrated `Add admin catalog portal and R2 media workflow` commit on `main`; the exact SHA is the commit containing this section.
- External Hostinger backend/frontend were not uploaded, production migrations were not run, and Cloudflare R2 bucket/CORS/custom-domain/credentials were not externally configured. Code and deployment documentation are ready for that separate authorized deployment. No seller offers, inventory, carts, orders, payments, reviews, recipes, public catalog switch, backend ZIP, or frontend ZIP was added; the next phase can build on the completed admin catalog boundary.
...
- Hierarchies allow four levels (depth `0..3`); one service prevents self-parenting, direct/indirect cycles, descendant moves, and excessive subtree depth. Normal removal archives; restore returns to draft; permanent deletion requires archive state, a standard Cancel/Delete modal, and no active or soft-deleted children.
...
- Completed the previously partial Laravel 12 skeleton with `public/index.php`, `phpunit.xml`, `tests/`, a user factory, `composer.lock`, and queue-table migrations. The locked framework is Laravel 12.63.0; local validation used PHP 8.3.32 and Composer 2.10.2, satisfying PHP `^8.2`.
- Preserved the public bearer contract at register/login/me/logout and the current `customer`, `supplier`, and `partner` account types. Registration now requires the password the current frontend already sends. Passwords use Laravel hashing; only SHA-256 token digests are stored.
- New user tokens expire after 30 days and new admin tokens after 8 hours by default. Existing deployed tokens with `expires_at = null` remain valid for compatibility. Logout revokes the current token; password changes revoke other active sessions.
- Public users and admins remain separate through distinct token tables and middleware. Added the requested `/api/v1/admin/dashboard` alias without removing `/dashboard-stats`, and reserved an empty `/api/v1/catalog` group for Step 2.
- Added named API, registration, public-login, and admin-login rate limiters; safe JSON 404 responses; exact CORS support for `https://foodonlines.com` and `https://www.foodonlines.com`; redacted mail-failure logging; and `GET /api/v1/health`.
- Existing addresses, notification preferences, masked payment metadata, password changes, account deletion requests, admin user listings/settings/dashboard statistics, and customer/supplier/partner persistence remain intact. No category, product, inventory, order, payment-processing, R2, or other ecommerce module was added.
- Profile-update inventory: admin name/email/password updates exist at `PUT /api/v1/admin/settings` and are preserved. No standalone public `PUT /account/profile` route or frontend API call existed at audit time, so Step 1 did not invent a new public contract.
- Migration audit: 11 migrations apply cleanly and report `Ran` on a temporary SQLite database. The two historical compatibility migrations intentionally retain non-destructive rollback behavior; the new token-expiration and queue migrations have tested rollback definitions.
- Verification: Composer validation passed; Composer security audit found no vulnerable/advisory packages; PHP syntax passed; all migrations rolled back/reapplied cleanly; 27 `/api/v1` routes registered; and 11 PHPUnit tests passed with 60 assertions across health, CORS/error safety, public auth, token expiry, admin separation, logout revocation, and rate limits. Frontend TypeScript/build and the existing 1,032-file production reference audit also passed with no frontend source or deployment-mirror diff.
- Live backend deployment was not possible in this session. No Hostinger backend credentials, File Manager connector, SSH, or deploy hook is available, and `www.api.foodonlines.com` did not resolve from the local DNS check. Git is the synchronized source of truth; production still requires upload/pull, Composer install, migrations, cache rebuild, and live smoke tests before Step 2 begins.
- Backend operating guide: `docs/backend-foundation.md`. Current non-ZIP Hostinger workflow: `DEPLOYMENT.md`; the isolated API public entry is `deployment/hostinger/backend-public/` because root `public/.htaccess` must remain the frontend SPA rewrite contract. Do not create a backend deployment ZIP for Step 1.
...
- For repository tasks, the "live deployment" copy means `frontend-upload/`: keep it synchronized with the exact current `dist/` output plus `.htaccess` and deployment instructions. An actual Hostinger upload must only be claimed when File Manager evidence exists.
...
- Static checks passed. Optimization commit `83defef` was pushed to `origin/main`; `frontend-upload/` was then rebuilt from the same optimized `dist/` output so the Git source and repository live-deployment mirror are synchronized.
...
- Frontend API config source is `src/lib/runtimeConfig.ts`. Default production API base URL is `https://api.foodonlines.com/api/v1`. Production build now uses relative Vite asset paths for safe cPanel subfolder upload.
...
- TMDHosting packaging auth endpoints preserved: `POST https://api.foodonlines.com/api/v1/auth/register`, `POST https://api.foodonlines.com/api/v1/auth/login`, `GET https://api.foodonlines.com/api/v1/auth/me`, `POST https://api.foodonlines.com/api/v1/auth/logout`, `POST https://api.foodonlines.com/api/v1/admin/login`, and `GET https://api.foodonlines.com/api/v1/admin/users?account_type=...`.
...
- TMDHosting packaging test URLs: `https://foodonlines.com/`, `https://foodonlines.com/admin`, `https://foodonlines.com/admin.html`, `https://foodonlines.com/#login`, `https://foodonlines.com/#signup`, and `https://api.foodonlines.com/api/v1/auth/me`.
...
- Admin API connection fix on 2026-05-19: admin login now calls `https://api.foodonlines.com/api/v1/admin/login` through shared API base `https://api.foodonlines.com/api/v1`, using JSON payload `{ email, password }`. No visual design changes made. Historical raw diagnostics are superseded by the 2026-07-17 safe error-normalization rule.
...
- Public/live auth endpoints preserved in this package: `POST https://api.foodonlines.com/api/v1/auth/register`, `POST https://api.foodonlines.com/api/v1/auth/login`, `GET https://api.foodonlines.com/api/v1/auth/me`, `POST https://api.foodonlines.com/api/v1/auth/logout`, `POST https://api.foodonlines.com/api/v1/admin/login`, and `GET https://api.foodonlines.com/api/v1/admin/users?account_type=...`.
- Shared runtime API base source remains `src/lib/runtimeConfig.ts` with production default `https://api.foodonlines.com/api/v1`. Do not point frontend auth to the public frontend domain or localhost.
...
- Package test URLs: `https://foodonlines.com/`, `https://foodonlines.com/admin`, `https://foodonlines.com/admin.html`, `https://foodonlines.com/#login`, `https://foodonlines.com/#signup`, and `https://api.foodonlines.com/api/v1/auth/me`.
...
- Removed the shared footer's Bangkok office address, Call Us phone number, and business hours across every public page.
- The footer contact area now displays only `Email: sale@foodonlines.com`; unused location, phone, and hours icon branches and contact-item types were removed.
