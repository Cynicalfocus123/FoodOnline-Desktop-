...
Refer & Earn is Laravel-authoritative. Only Customers receive an immutable active code; invite resolution is read-only; registration attributes a valid other-customer code exactly once; referral rewards create account-bound promotions; qualification is idempotent; and a full refund revokes an unused reward or moves a redeemed reward to review. Customer dashboard, activity, and coupons have separate loading/error/retry states. Supplier and Partner accounts receive no customer referral UI. Administrator Referral operations uses server-backed search, filters, pagination, detail actions, settings, audit history, and safe empty states; it must never fall back to demo rows.
...
The 2026-07-22 referral restoration passed the dedicated Laravel suite (6 tests / 74 assertions), 101 Node checks, TypeScript no-emit, and a controlled isolated acceptance run. That run migrates a temporary SQLite database, verifies Customer code/invite/registration/activity/coupon ownership, Admin list/detail, compiled direct-route refresh, and delivered/COD qualification plus full-refund revocation. It terminates its temporary Laravel, static, and browser processes. The address acceptance gates remain mandatory. External Hostinger deployment, production migration, and live smoke testing remain unperformed and unverified.
