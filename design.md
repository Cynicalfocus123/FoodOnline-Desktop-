...
Restricted administrators keep the full sidebar visible for orientation, but modules outside their effective grants are greyed out, marked “No access,” disabled for mouse and keyboard navigation, and blocked on direct routes. The Staff & MFA workspace is visible only to Super Admins and provides safe account creation, preset-role defaults, per-person permission overrides, status changes, password resets, and session revocation without displaying secrets.
