...
Restricted administrators keep the full sidebar visible for orientation, but modules outside their effective grants are greyed out, marked “No access,” disabled for mouse and keyboard navigation, and blocked on direct routes. Admin Settings contains the Super Admin-only administrator accounts section: a responsive table of created admins plus safe account creation, preset-role defaults, per-person permission overrides, status changes, password resets, and session revocation without displaying secrets. Product management is separate from inventory management through the dedicated `product_manager` role; the existing `catalog_manager` role manages products and brands only. The Staff & MFA sidebar item remains a shortcut to the same workspace.
