...
- The About Us circle assets live in `public/images/about/circle-assets/` as transparent PNGs. Most food assets are cropped cutouts, but the delivery truck must keep the original full transparent canvas from `9.Our teams fulfill and deliver more than 100,000 orders every day (1).png` so no part of the vehicle is cut off. Images should stay `object-contain`, lazy-loaded, non-stretching, and sized with responsive Tailwind classes so desktop, Chrome, Safari, Android, iPhone, mobile, and desktop-mobile browser widths keep the cutouts overstepping the circles without creating horizontal page overflow.
