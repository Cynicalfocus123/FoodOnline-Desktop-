...
- Public login now preserves the pre-login route for the existing email auth path, so successful sign-in can return to product/category/search/cart/checkout context instead of always dropping back at home. No real phone SMS verification backend exists in this workspace yet, so no fake OTP flow was added.
