...
{
    public function index(Request $request): JsonResponse { $data = $request->validate(['status' => ['nullable', Rule::in(ReturnRequest::STATUSES)], 'search' => ['nullable', 'string', 'max:80'], 'per_page' => ['nullable', 'integer', 'min:1', 'max:100']]); $query = ReturnRequest::query()->with(['user', 'order', 'items.orderItem'])->latest(); if (! empty($data['status'])) $query->where('status', $data['status']); if (! empty($data['search'])) $query->where(fn ($q) => $q->where('return_number', 'like', '%'.$data['search'].'%')->orWhereHas('order', fn ($o) => $o->where('order_number', 'like', '%'.$data['search'].'%'))); return response()->json(['data' => $query->paginate($data['per_page'] ?? 25)]); }
