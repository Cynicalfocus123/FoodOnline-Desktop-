...
{
    public function index(Request $request): JsonResponse { return response()->json(['data' => $request->user()->supportTickets()->with('messages')->latest()->paginate(20)]); }
    ...
    public function show(Request $request, SupportTicket $ticket): JsonResponse { abort_unless($ticket->user_id === $request->user()->id, 404); return response()->json(['ticket' => $ticket->load('messages')]); }
