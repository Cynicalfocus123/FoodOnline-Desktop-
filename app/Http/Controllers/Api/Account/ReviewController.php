...
{
    public function index(Request $request): JsonResponse { return response()->json(['data' => $request->user()->reviews()->with(['product', 'media'])->withCount('votes')->latest()->get()->map(fn ($review) => ReviewPresenter::make($review))]); }
