...
export const countries: Country[] = countryCodes
  .map((code) => ({ code, name: preferredNames[code] ?? names?.of(code) ?? code }))
  .sort((a, b) => a.name.localeCompare(b.name, "en"));

const countriesByCode = new Map(countries.map((country) => [country.code, country.name]));

export function countryNameFromCode(code: string | null | undefined): string {
  const normalized = code?.trim().toUpperCase() ?? "";
  return countriesByCode.get(normalized) ?? normalized;
}
