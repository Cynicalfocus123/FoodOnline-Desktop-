const technicalContentPatterns = [
  ...
  /\bwww\.[a-z0-9.-]+/i,
  /\/api\/v\d+(?:\/|\b)/i,
  /\b(?:api\s+(?:target|base)|(?:backend|server)\s+url|endpoint|sqlstate|mysql|postgres(?:ql)?|laravel|php\s+error|stack\s+trace|traceback|exception)\b/i,
  /\b(?:app_key|db_(?:host|database|username|password)|aws_(?:access|secret)|r2_(?:access|secret|endpoint)|vite_api_base_url)\b/i,
