...
export function EnterpriseUsersAdminPanel({ token, role, users, loading, mode = "list", recordId, onChangeRole, onNavigate, onReload }: Props) {
  ...
  const filtered = useMemo(() => users
    .filter((user) => (!status || (status === "active" ? user.requestStatus === "approved" : status === "disabled" ? user.requestStatus === "disabled" : user.requestStatus === "in_review")) && `${user.emailAddress} ${user.firstName} ${user.lastName} ${user.contactNumber} ${user.lineId} ${user.companyName}`.toLowerCase().includes(search.toLowerCase()))
