...
const staffRoles = ["super_admin", "catalog_manager", "custom", "order_manager", "inventory_manager", "customer_support", "marketing_manager", "read_only"] as const;
...
export function StaffAdminPanel({ token }: { token: string }) {
  ...
  const [createForm, setCreateForm] = useState({ name: "", email: "", password: "", password_confirmation: "", status: "active" });
  ...
  const createStaff = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await operationsApi.createStaff(token, { ...createForm, staff_role: "read_only", staff_permissions: staffRoleDefaults.read_only });
      setCreateForm({ name: "", email: "", password: "", password_confirmation: "", status: "active" });
  ...
  return <Shell title="Staff & MFA administration">
    ...
    <div className="mt-6 grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
      <form className="rounded-3xl border border-neutral-200 p-5" onSubmit={createStaff}>
        ...
        <div className="mt-4 grid gap-3">
          {([["name", "Display name", "text"], ["email", "Email address", "email"], ["password", "Password", "password"], ["password_confirmation", "Confirm password", "password"]] as const).map(([key, label, type]) => <label className="grid gap-1" key={key}><span className="text-sm font-bold text-neutral-700">{label}</span><input className="min-h-11 rounded-2xl border border-neutral-200 px-3 outline-none focus:border-leaf-500 focus:ring-2 focus:ring-leaf-500/15" autoComplete={key === "email" ? "username" : key.startsWith("password") ? "new-password" : "name"} required type={type} value={createForm[key]} onChange={(event) => setCreateForm((current) => ({ ...current, [key]: event.target.value }))} /></label>)}
           ...
           <p className="text-xs leading-5 text-neutral-500">New administrators start with read-only access. Click <span className="font-bold">Edit account</span> in the table to assign the role and permissions.</p>
      ...
      <div className="rounded-3xl border border-neutral-200 p-5">
        ...
        <div className="mt-4 overflow-x-auto rounded-2xl border border-neutral-200"><table className="min-w-[900px] w-full border-collapse text-left text-sm"><thead className="bg-neutral-50"><tr><th className="px-4 py-3 text-xs font-black uppercase tracking-[0.12em] text-neutral-500">Administrator</th><th className="px-4 py-3 text-xs font-black uppercase tracking-[0.12em] text-neutral-500">Role</th><th className="px-4 py-3 text-xs font-black uppercase tracking-[0.12em] text-neutral-500">Status</th><th className="px-4 py-3 text-xs font-black uppercase tracking-[0.12em] text-neutral-500">MFA</th><th className="px-4 py-3 text-xs font-black uppercase tracking-[0.12em] text-neutral-500">Last login</th><th className="px-4 py-3 text-xs font-black uppercase tracking-[0.12em] text-neutral-500">Access</th><th className="px-4 py-3 text-right text-xs font-black uppercase tracking-[0.12em] text-neutral-500">Action</th></tr></thead><tbody>{loading ? <tr><td className="px-4 py-6 text-neutral-500" colSpan={7}>Loading administrator accounts...</td></tr> : rows.length === 0 ? <tr><td className="px-4 py-6 text-neutral-500" colSpan={7}>No administrator accounts found.</td></tr> : rows.map((row) => <tr className="border-t border-neutral-100 bg-white" key={String(row.id)}><td className="px-4 py-3"><strong className="block">{String(row.name)}</strong><span className="block break-all text-xs text-neutral-500">{String(row.email)}</span></td><td className="px-4 py-3 font-semibold">{roleLabel(String(row.staff_role))}</td><td className="px-4 py-3"><span className="rounded-full bg-neutral-100 px-2 py-1 text-xs font-black">{String(row.status)}</span></td><td className="px-4 py-3">{row.mfa_enabled ? "Enabled" : "Not enabled"}</td><td className="px-4 py-3 text-neutral-600">{row.last_login_at ? String(row.last_login_at) : "Not yet"}</td><td className="px-4 py-3">{String(row.permission_count ?? 0)} permissions</td><td className="px-4 py-3 text-right"><button className="rounded-full border border-leaf-500 px-3 py-2 text-xs font-black text-leaf-700" onClick={() => choose(row)} type="button">Edit account</button></td></tr>)}</tbody></table></div>
      </div>
    </div>
    {selected ? <div aria-labelledby="edit-admin-title" aria-modal="true" className="fixed inset-0 z-50 overflow-y-auto bg-black/40 px-4 py-8" role="dialog"><div className="mx-auto grid max-w-5xl gap-6 rounded-3xl border border-neutral-200 bg-white p-5 shadow-2xl xl:grid-cols-[1fr_0.8fr]"><div><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-citrus-500">Administrator editor</p><h3 className="mt-2 text-xl font-black" id="edit-admin-title">Edit administrator account</h3><p className="mt-1 text-sm text-neutral-600">{String(selected.name)} · {String(selected.email)}</p></div><button aria-label="Close administrator editor" className="rounded-full border px-4 py-2 text-sm font-black" onClick={() => setSelected(null)} type="button">Close editor</button></div><div className="mt-5 grid gap-3 sm:grid-cols-2"><label className="grid gap-1"><span className="text-sm font-bold text-neutral-700">Preset role</span><select className="min-h-11 rounded-2xl border px-3" value={role} onChange={(event) => { setRole(event.target.value); setPermissions(staffRoleDefaults[event.target.value] ?? []); }}>{staffRoles.map((item) => <option key={item} value={item}>{roleLabel(item)}</option>)}</select></label><label className="grid gap-1"><span className="text-sm font-bold text-neutral-700">Status</span><select className="min-h-11 rounded-2xl border px-3" value={status} onChange={(event) => setStatus(event.target.value)}><option value="active">Active</option><option value="disabled">Disabled</option></select></label></div><div className="mt-4"><p className="text-sm font-black">Access permissions</p><p className="mt-1 text-xs leading-5 text-neutral-500">Checked permissions are visible to this administrator. Manage permissions automatically include read access.</p><div className="mt-2 grid max-h-72 gap-2 overflow-y-auto rounded-2xl bg-neutral-50 p-3 sm:grid-cols-2">{staffPermissionOptions.map((permission) => <label className="flex min-h-9 items-center gap-2 text-sm" key={permission}><input checked={permissions.includes(permission)} onChange={() => togglePermission(permission)} type="checkbox" /><span>{permissionLabel(permission)}</span></label>)}</div></div><button className="mt-4 rounded-full bg-leaf-600 px-5 py-3 text-sm font-black text-white disabled:bg-neutral-300" disabled={saving} onClick={() => void saveStaff()} type="button">Save role and permissions</button></div><form className="rounded-2xl bg-neutral-50 p-4" onSubmit={(event) => { event.preventDefault(); void resetStaffPassword(); }}><h3 className="font-black">Reset password</h3><p className="mt-1 text-sm text-neutral-600">All active sessions for this administrator will be revoked.</p><div className="mt-3 grid gap-3"><input className="min-h-11 rounded-2xl border px-3" autoComplete="new-password" minLength={10} placeholder="New password" required type="password" value={resetPassword} onChange={(event) => setResetPassword(event.target.value)} /><input className="min-h-11 rounded-2xl border px-3" autoComplete="new-password" minLength={10} placeholder="Confirm new password" required type="password" value={resetConfirmation} onChange={(event) => setResetConfirmation(event.target.value)} /><button className="rounded-full border border-leaf-500 px-4 py-3 text-sm font-black text-leaf-700 disabled:opacity-50" disabled={saving} type="submit">Reset password</button></div></form></div></div> : null}
